...
void updateSerialNumberFromBMC( TARGETING::Target * i_nodetarget )
{
	...
	if (l_sn_prod != NULL)
	{
		CONSOLE::displayf(NULL, "Got system serial number from BMC.");
		CONSOLE::flush();
		TRACFCOMP(g_trac_vpd, "Got system serial number from BMC.");
		CONSOLE::displayf(NULL, "SN from BMC is: %s", l_sn_prod);
		CONSOLE::flush();
		...
		if(l_errl == NULL)
		{
			...
			if(l_errl == NULL)
			{
				CONSOLE::displayf(NULL, "SN in PVPD::OSYS:SS: %s, size: %d", l_vpddata, l_vpdSize);
				CONSOLE::flush();
		...
		CONSOLE::displayf(NULL, "End updateSerialNumberFromBMC.");
		CONSOLE::flush();
	}
    #endif
#endif
